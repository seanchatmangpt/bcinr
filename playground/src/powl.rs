...
use bcinr::mask::{select_u64, select_u32};
...
pub fn powl64_execute_step(
    ...
    // For LoopGate:
    let is_exit_loop_mask = 0u64.wrapping_sub(((op.ctrl_mask | op.ctrl_mask.wrapping_neg()) >> 63) ^ 1);
    ...
    let loop_ops = select_u64(exit_and_repeat_mask, state.completed_ops & !op.pred_mask, state.completed_ops);
    ...
    let next_completed_branches_raw = select_u64(is_choice_mask, choice_branches, state.completed_branches);
    state.completed_branches = select_u64(exec_mask, next_completed_branches_raw, state.completed_branches);
    ...
    let enter_loop_loops = select_u64(!should_repeat_mask, state.completed_loops | loop_bit, state.completed_loops);
    // Exit loop gate: if repeat, clear loop completed status, else set loop completed status.
    let exit_loop_loops = select_u64(should_repeat_mask, state.completed_loops & !loop_bit, state.completed_loops | loop_bit);
    ...
    let depth_change = select_u32(
        exec_mask as u32,
        select_u32(is_enter_scope_mask as u32, 1, select_u32(is_exit_scope_mask as u32, 0xFFFF_FFFF, 0)),
        0
    ) as u32;
        ...
        match &node.kind {
            ...
            PowlNodeKind::PartialOrder(_) => {
                ...
                for edge in &powl.edges {
                    if op_count >= out.len() { return Err("Output buffer too small"); }
                    out[op_count] = Powl64Op {
