...
pub fn powl2_language(model: &Powl2Model, max_len: usize) -> Language {
    match model {
        ...
        Powl2Model::PartialOrder { children, edges } => {
            let child_langs: Vec<Language> = children.iter().map(|c| powl2_language(c, max_len)).collect();
            let n = children.len();
        ...
        Powl2Model::ChoiceGraph { children, edges, start, end } => {
            let child_langs: Vec<Language> = children.iter().map(|c| powl2_language(c, max_len)).collect();
            choice_graph_language(&child_langs, edges, *start, *end, children.len(), max_len)
        }
        Powl2Model::DoRedo { body, redo, max_redos } => do_redo_language(body, redo, *max_redos, max_len),
    }
}
...
fn order_preserving_shuffle(child_langs: &[Language], prec: &[Vec<bool>], max_len: usize) -> Language {
...
fn interleave(combo: &[Vec<String>], prec: &[Vec<bool>], n: usize, max_len: usize, result: &mut Language) {
...
/// Bound on how many times a choice-graph node may be *stalled on* --
/// re-entered with no net trace-length growth since its previous entry on
/// the current DFS path -- when unrolling cyclic paths for language
/// enumeration. A node re-entered only after genuine progress (the prefix
/// grew since that node's last visit) does not consume this budget: it is
/// bounded instead by `max_len` via the existing `prefix.len()` cap. This
/// keeps epsilon-only hops (e.g. a `Powl2Model::Silent` child, whose
/// language is `{[]}`) from starving out real progressing walks that
/// happen to revisit the same node more than `MAX_CYCLE_UNROLL` times.
...
fn choice_graph_language(
    ...
    let max_node = edges.iter().flat_map(|&(a, b)| [a, b]).max().unwrap_or(end).max(end).max(start);
    ...
    let mut budgets = vec![NodeBudget { last_progress: None, stall: 0 }; max_node + 1];
    cg_dfs(&adj, child_langs, start, end, n_children, max_len, &mut prefix, &mut budgets, &mut result);
    result
}
...
/// counter exactly like the old raw-revisit counter did, and is pruned once
...
fn cg_dfs(
    ...
    budgets[node] = NodeBudget { last_progress: Some(cur_len), stall: new_stall };
    if node < n_children {
        for seq in &child_langs[node] {
            ...
            for &nxt in &adj[node] {
                cg_dfs(adj, child_langs, nxt, end, n_children, max_len, prefix, budgets, result);
            }
...
/// just the maximum).
fn do_redo_language(body: &Powl2Model, redo: &Powl2Model, max_redos: u8, max_len: usize) -> Language {
