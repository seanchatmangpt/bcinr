//! Deterministic capability router: a minimal viable slice proving that
//! "which AI surface should handle this task" can be a decidable admission +
//! scheduling question instead of a routing-table heuristic.
//!
//! A capability (Claude Code editing a repo file, Claude Chrome filling a web
//! form, Claude Desktop drafting text) is modeled as a durative action; the
//! shared human/session resource it competes for (`attention` — one working
//! human, one active train of thought) is a numeric fluent; two capabilities
//! that would touch the same file are modeled as both requiring an exclusive
//! `locked` predicate on that file. This reuses `find_temporal_plan` and
//! `analyze_schedule` unchanged — this module contributes exactly one new
//! thing: a [`CostVector`] with a deterministic lexicographic ordering to
//! pick between routes `analyze_schedule` reports as equally schedulable.
//!
//! Fixed capability set for this slice (not a general plugin graph — see
//! `docs/` for why growing this is a deferred follow-up):
...
/// satisfied one the instant its resource frees up (a real interaction
/// discovered while building this router, not a planner bug worth patching
/// for one caller — the guard is the correct PDDL modeling fix).
...
/// This is the one genuinely new piece of logic in this slice — no cost
/// vector or lexicographic ordering exists elsewhere in bcinr-pddl/bcinr-powl.
...
/// fixed capability set. Returns `Err` (not a silent default route) when
/// the task is infeasible under the given attention capacity — the
