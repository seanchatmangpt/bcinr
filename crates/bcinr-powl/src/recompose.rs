...
/// Recompose a `Powl2Model` into an equivalent safe & sound WF-net.
#[must_use]
pub fn recompose(model: &Powl2Model) -> WfNet {
...
impl Builder {
    ...
    fn build(&mut self, model: &Powl2Model) -> (String, String) {
        match model {
            ...
            Powl2Model::PartialOrder { children, edges } => self.build_partial_order(children, edges),
    ...
    fn build_partial_order(&mut self, children: &[Powl2Model], order: &[(usize, usize)]) -> (String, String) {
        let n = children.len();
