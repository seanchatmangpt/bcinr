...
fn branchless_static_evaluation(pieces: &[Piece]) -> i32 {
    ...
    for p in pieces {
        if p.alive {
            ...
            if p.color == "White" { white_score += val; } 
            else { black_score += val; }
        }
    }
...
fn main() {
    ...
    let setup = [
        ("Rook_a", "Rook", 0), ("Knight_b", "Knight", 1), ("Bishop_c", "Bishop", 2), ("Queen", "Queen", 3),
        ("King", "King", 4), ("Bishop_f", "Bishop", 5), ("Knight_g", "Knight", 6), ("Rook_h", "Rook", 7),
        ("Pawn_a", "Pawn", 8), ("Pawn_b", "Pawn", 9), ("Pawn_c", "Pawn", 10), ("Pawn_d", "Pawn", 11),
        ("Pawn_e", "Pawn", 12), ("Pawn_f", "Pawn", 13), ("Pawn_g", "Pawn", 14), ("Pawn_h", "Pawn", 15),
    ];

    for (name, kind, pos) in setup.iter() {
        pieces.push(Piece { id: format!("White_{}", name), kind, color: "White", alive: true, square: *pos });
    }
    for (name, kind, pos) in setup.iter() {
        let real_pos = if *pos >= 8 { 48 + (*pos - 8) } else { 56 + *pos };
        pieces.push(Piece { id: format!("Black_{}", name), kind, color: "Black", alive: true, square: real_pos }); 
    }
    ...
    for i in 0..64 {
        ...
        objects.push(format!(r#"    {{ "id": "{}{}", "type": "Square", "attributes": [] }}"#, file, rank));
    }
    for p in &pieces {
    ...
    while !game_over && turn_counter <= 100 {
        ...
        for mv in &legal_moves {
            ...
            if let Some(v_idx) = test_pieces.iter().position(|p| p.alive && p.square == mv.1 && p.color != color_filter) {
            ...
            if mv == &legal_moves[0] {
                eprintln!("Turn {:03} | NNUE Guess: {:>5} | Shannon Target: {:>5} | Error: {:>5}", turn_counter, nnue_pred, shannon_target, (nnue_pred - shannon_target).abs());
            }
            ...
            if is_white && shannon_target > best_score {
                best_score = shannon_target; best_move = *mv;
        ...
        let target_square_str = format!("{}{}", (b'a' + (target_square % 8) as u8) as char, (target_square / 8) + 1);
        ...
        if let Some(victim_idx) = pieces.iter().position(|p| p.alive && p.square == target_square) {
            ...
            relationships.push(format!(r#"{{ "objectId": "{}", "qualifier": "captured_piece" }}"#, pieces[victim_idx].id));
