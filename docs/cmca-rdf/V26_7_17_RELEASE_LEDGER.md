...
## Publication Sequencing Decision — 2026-07-18 (Track B+E, packaging-reality + local-registry-rehearsal pass)

This section is appended by the Track B+E pass (packaging-reality verification artifact +
publication-sequencing investigation), scoped to `Makefile.toml`, `scripts/gates/`, and this
ledger only. It does not edit, retire, or reinterpret any REPORTED-era or dated-addendum entry
above; it records what this pass found, additively.

### Committed packaging-reality artifact (Track B)

The earlier ad-hoc `cargo package`/`cargo publish --dry-run` commands cited in this ledger's
"cmca-release-integrator terminal pass" section above were run by hand and reported in prose —
a later independent verifier correctly could not mechanically reproduce them from that prose
alone. This pass adds a replayable, committed artifact closing that gap:

- `scripts/gates/package-reality-check.sh` — the mechanics: packages `bcinr-logic`, attempts
  `bcinr-cmca`, classifies the failure (known sequencing blocker below vs. anything
  UNEXPECTED), extracts+tests any produced `bcinr-cmca` tarball from its own packaged contents
  only, digests every `.crate` file under `target/package/`, and states the mfw
  filesystem-absence limitation honestly (see below). Exit 0 = no unexpected failure; exit 1 =
  unexpected failure.
- `cargo make package-reality-check` (new `Makefile.toml` task) — runs that script and
  regenerates `crates/bcinr-cmca/PACKAGE_REALITY_RECEIPT.md` in full from this run's own
  output. The receipt is explicitly headed "MUTABLE RECEIPT — NOT A RULE": it is this run's
  observed output, not a standing invariant, per this repo's control-plane separation
  (`.claude/rules/*.md` = timeless rules; receipts and this ledger = mutable evidence).

**Real run, this pass** (`cargo make package-reality-check`, branch `recovery/cmca-v26.7.17-c2`,
working tree dirty — 23 uncommitted/untracked paths at run time, all attributable to concurrent
in-progress work on this same branch, not to this pass): `bcinr-logic` packaged successfully
without needing `--allow-dirty` (none of the dirty paths are inside `crates/bcinr-logic/`).
`bcinr-cmca` needed an `--allow-dirty` retry (its own directory has uncommitted files from
concurrent work) and then failed with exactly the known sequencing blocker text below — no
`--no-verify`/`--allow-dirty` result here is treated as release-closing evidence, only as an
interim smoke check, per `.claude/rules/cmca/packaging.md`. No `bcinr-cmca` tarball was produced
this run, so step (c) (extract+test from packaged contents) was SKIPPED, as designed. Full
output is in `crates/bcinr-cmca/PACKAGE_REALITY_RECEIPT.md` (regenerated by this run) —
including the recorded sha256 digests of `target/package/bcinr-logic-26.7.17.crate` (this run)
and a stale pre-existing `target/package/bcinr-cmca-26.6.24.crate` (a prior version's leftover,
correctly labeled STALE and left in place, not deleted).

### The three lawful options for the bcinr-logic/bcinr-cmca sequencing blocker

Per `.claude/rules/cmca/packaging.md` and the exact failure text recorded in this ledger's
"Package / publish dry-run" section above (`failed to select a version for the requirement
` + "`bcinr-logic = \"^26.7.17\"`" + `, ... location searched: crates.io index`), three lawful
paths exist to close this blocker:

1. **Publish `bcinr-logic` 26.7.17 to crates.io for real, then dry-run `bcinr-cmca`.** Once
   `bcinr-logic` 26.7.17 (or higher) exists on the real crates.io index, `cargo package -p
   bcinr-cmca --locked` / `cargo publish -p bcinr-cmca --dry-run --locked` will resolve the
   `bcinr-logic = "^26.7.17"` requirement against the real registry and can be re-attempted.
   This is the only path that produces genuine release-closing evidence for `bcinr-cmca`'s
   packaging, because it removes the blocker at its actual source. It requires an irreversible,
   externally-visible `cargo publish` of `bcinr-logic` — explicitly out of this pass's
   authorization (see below).
2. **Local-registry rehearsal as interim evidence.** Stand up a local package registry
   containing `bcinr-logic` 26.7.17, point the `bcinr-cmca` packaging step at it instead of
   crates.io, and observe whether `cargo package -p bcinr-cmca` then succeeds. This would be
   interim, non-release-closing evidence only (packaging.md: any override that substitutes for
   real registry state is, at best, a smoke check) — but genuine, real evidence that the
   *dependency shape itself* is not the problem, only the publish-ordering. Attempted this pass;
   see the next section for the real result.
3. **Change the release topology** to avoid the path-dependency-version mismatch entirely — for
   example, by not requiring `bcinr-cmca`'s packaged manifest to carry a registry-resolvable
   version requirement on `bcinr-logic` until `bcinr-logic` is actually published (e.g.
   deferring `bcinr-cmca`'s own publish to a later release cycle after `bcinr-logic` lands on
   crates.io), or restructuring so `bcinr-cmca` is not itself published until this ordering is
   satisfied. This is a scope/policy decision for `cmca-release-integrator`, not something this
   pass decides or recommends over the other two — it is named here only so it is not silently
   assumed off the table.

**Statement, per this ledger's existing discipline:** actual `cargo publish` (without
`--dry-run`) of `bcinr-logic` — option 1 — is **NOT performed in this pass**. It is an
irreversible, externally-visible action outside the scope of this workflow and requires
explicit separate authorization, exactly as this ledger's "cmca-release-integrator terminal
pass" section above already stated for the same reason. This pass did not attempt it, and does
not recommend a specific option among the three — it records them as the lawful option set for
`cmca-release-integrator` to decide among.

### Local-registry rehearsal (Track E): what was attempted and the real result

**Tooling check (real commands, this pass):**

```
$ which cargo-local-registry
(not found)
$ cargo local-registry --help
error: no such command: `local-registry`
```

Neither `cargo-local-registry` nor an equivalent `cargo local-registry` subcommand is installed
in this environment.

**Network reachability check (real commands, this pass, to assess whether installing the
tooling was even feasible):**

```
$ curl -s -m 5 -o /dev/null -w "%{http_code}\n" https://static.crates.io/crates/bcinr-logic
403
$ curl -s -m 5 -o /dev/null -w "%{http_code}\n" https://index.crates.io/config.json
200
```

The sparse-index metadata endpoint (`index.crates.io`) is reachable, but the actual crate-file
CDN (`static.crates.io`) returns 403 in this environment. `cargo install cargo-local-registry`
would need to download that tool's own source `.crate` from `static.crates.io`, so installing it
was assessed as very likely to fail outright and was not attempted (no value in a doomed install
attempt burning the pass's time budget).

**Lighter-weight alternative attempted instead: a temporary `[patch.crates-io]` local-path
override for `bcinr-logic` in the workspace root `Cargo.toml`, empirically tested and
reverted.** This did not require any network access or any new tooling, and directly tests one
of this ledger's own open items — G1 hypothesis #2 ("does a local-path patch override actually
block or fix dry-run packaging"), applied here to `bcinr-logic` specifically rather than
`wasm4pm-compat`. Real commands, this pass (workspace root `Cargo.toml` restored to its original
committed content immediately after; `git diff Cargo.toml` confirmed empty afterward):

```
$ # [patch.crates-io] temporarily gained: bcinr-logic = { path = "crates/bcinr-logic" }
$ cargo package -p bcinr-cmca --locked --no-verify --allow-dirty
...
**Result: identical failure, patch present or not.** This is real, useful negative evidence,
not merely an assumption from Cargo's general documentation: `[patch.crates-io]` overrides in
the workspace root manifest are **not** consulted during `cargo package`'s dependency-resolution
step for the crate being packaged — the packaging/verification step resolves against the true
registry only. This independently confirms (for `bcinr-logic`, empirically, in this exact
repository) the same shape of concern this ledger's G1 hypothesis #2 already raised for
`wasm4pm-compat`: an existing local-path `[patch]` entry cannot be assumed to make `cargo
package`/`cargo publish --dry-run` succeed, on this machine or any other, for the package it
patches. It does not, by itself, resolve G1 hypothesis #2 for `wasm4pm-compat` specifically
(that patch was not touched or tested by this pass), but it is directly relevant supporting
evidence for the same question and should be read alongside it.

**Full local-registry rehearsal (wholesale `[source.crates-io] replace-with` via a temporary
`.cargo/config.toml`, the remaining sanctioned override mechanism that does not require editing
any file outside this pass's owner scope): assessed and NOT attempted — skipped honestly rather
than forced.** Reasoning: `cargo package`'s dependency resolution for the temporary packaging
manifest is not limited to `bcinr-cmca`'s direct dependency (`bcinr-logic`) — it also resolves
`bcinr-cmca`'s dev-dependencies (`trybuild`, `proptest`, `blake3`, `serde`, `serde_json`,
`chicago-tdd-tools`) and `bcinr-logic`'s own dependencies (`criterion`, `proptest`), confirmed
directly: `cargo package -p bcinr-cmca --locked --no-verify` fails with the `bcinr-logic`
version-selection error *before* reaching any dev-dependency resolution, so this pass could not
empirically observe whether dev-deps would *also* need registry entries — but wholesale
`replace-with` of the `crates-io` source (the only mechanism available without touching a
Cargo.toml outside this pass's scope) applies to the *entire* resolution, not just
`bcinr-logic`, so every one of those packages — and each of *their* transitive dependencies —
would need a correct, hand-built local-registry index entry (name/version/dependency
edges/features/cksum) for resolution to succeed at all. A quick sizing check against the
existing (dirty, real) `Cargo.lock`: 16 packages match just the direct name patterns
(`trybuild`/`proptest`/`blake3`/`serde`/`chicago-tdd-tools`/`criterion`) out of 454 total
workspace lockfile entries, before counting transitive expansion (e.g. `trybuild` alone pulls in
`glob`, `termcolor`, `dissimilar`; `serde_json` pulls in `itoa`, `ryu`, `memchr`). Hand-building
correct index entries for that closure within this pass's time budget was judged heavy and
fragile — exactly the condition this task's own instructions named as grounds to skip rather
than force a workaround. **Skipped, per that explicit allowance, with this reasoning recorded
rather than a fabricated or partial result.**

Editing `crates/bcinr-cmca/Cargo.toml` to add a scoped `registry = "..."` field on just the
`bcinr-logic` dependency (which would avoid needing the whole crates-io source replaced, and so
would have been much lighter-weight) was considered and explicitly **not** done: that file is
outside this pass's owner scope (`Makefile.toml`, `scripts/gates/`,
`docs/cmca-rdf/V26_7_17_RELEASE_LEDGER.md` only) and, per `cmca-release-integrator`'s own role
definition, `crates/bcinr-cmca/Cargo.toml` is that agent's exclusive-authority file — this pass
does not touch it, even transiently.

### What this section does not claim

- It does not claim the local-registry rehearsal succeeded — it did not; the lighter-weight
  `[patch]` variant was tried and failed (informatively), and the full wholesale-replacement
  variant was sized and skipped, not attempted.
- It does not claim `[patch.crates-io]` overrides are useless in general — only that they are
  not consulted during `cargo package`'s own dependency-resolution/verification step, which is
  the specific, narrow, empirically-tested claim above.
- It does not claim `bcinr-logic` was published to crates.io, or that `cargo publish` was run
  for either crate, in this pass. Neither happened.
- It does not close G1 (Workspace/Packaging Hazards) or G0 (Release Identity) — both remain
  exactly as recorded in this ledger's "Gate Closure Summary" section above (G1 PARTIALLY
  REPRODUCED, open; G0 REPORTED, open). This section adds sequencing-decision documentation and
  one new empirical data point relevant to G1 hypothesis #2; it does not re-adjudicate either
  gate's standing.
