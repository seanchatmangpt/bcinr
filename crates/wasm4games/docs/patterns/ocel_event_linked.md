...
<!-- TODO: Describe the game situation that makes OcelEventLinked necessary.
...
<!-- TODO: List the tensions this pattern must hold in balance. Consider:
     - Branch misprediction: every OcelEventLinked call that branches adds jitter
     - Deterministic latency: the Mask lowering gives O(1) constant time
     - Bounded state: stateCard = 32 (32 distinct states)
     - Auditability: the OCEL event code 33 ties the transition to an object trace
     Authority to defend: oracle predicate: matches ocel_event_linked_reference for all inputs -->

_Replace this placeholder with the forces — what pulls in opposite directions here._

## Solution

<!-- TODO: Explain how ocel_event_linked resolves the forces.
...
graph LR
    state["state\n(u64)"]
    input["input\n(u64)"]
    kernel["ocel_event_linked\nMask: bcinr_logic::mask::select_u64"]
    result["result\n(u64)"]
    state --> kernel
    input --> kernel
    kernel --> result
