...
fn parse_derived(expr: &SExpr) -> Result<DerivedPredicate, Pddl8Error> {
    ...
    // The head is a typed parameter list, not a plain atom. Parsing it with
    // `parse_atom` stores the `-` separator and type name as literal extra
    // arguments, so `(ok ?x - obj)` became the 3-ary atom `ok(?x, -, obj)`;
    // grounding then emitted `ok(o1, -, obj)`, which can never match a use
    // site `(ok ?x)`. Parameterized, typed derived predicates silently never
    // fired. `parse_typed_names` is the same routine every other typed
    // parameter list in this file (`:parameters`, `:objects`) already uses.
