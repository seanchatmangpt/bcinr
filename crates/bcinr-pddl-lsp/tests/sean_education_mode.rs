...
use bcinr_pddl_lsp::education::{
    check_newsletter_need9, education_diagnostics, emit_education_domain, emit_education_problem,
    render_education_gate, render_education_lane, render_education_status, scan, EducationStage,
};
use bcinr_pddl_lsp::planner_client::{admit, plan};
...
fn full_fixture(dir: &TempDir) {
    ...
    write_file(
        dir,
        "docs/adr/0001-education-mode.md",
        "# ADR 0001\n\n## Status: ADMITTED\n",
    );
    write_file(
        dir,
    ...
    write_file(
        dir,
        "newsletter/issues/001-mechanical-planning.md",
        "# Newsletter\n\nREVIEWED\n\nSTATUS: PUBLISHED\n\nContent.\n",
    );
    write_file(
        dir,
    ...
    write_file(
        dir,
        "youtube/videos/001-rust-dfcm/outline.md",
        "# Outline\n\nContent.\n",
    );
    write_file(
        dir,
        "youtube/videos/001-rust-dfcm/script.md",
        "# Script\n\nContent.\n",
    );
    write_file(
        dir,
        "youtube/videos/001-rust-dfcm/recording.json",
    ...
    write_file(
        dir,
        "lessons/rust/001-bcinr-pddl-lsp.md",
        "# Lesson\n\nSTATUS: PUBLISHED\n\nContent.\n",
    );
    write_file(dir, "lessons/rust/examples/src/lib.rs", "pub fn foo() {}");
    write_file(
        dir,
        ".bcinr/test-report.json",
        r#"{"status":"passed","passed":true}"#,
    );
    write_file(
        dir,
        ".bcinr/ocel/latest.json",
        r#"{"event_count":5,"events":[]}"#,
    );
}
...
fn empty_education_workspace_is_blocked() {
    ...
    assert_eq!(
        ws.next_missing(),
        Some(&EducationStage::CareerPipelineExists)
    );
...
fn interview_request_requires_slot_before_confirmation() {
    ...
    fs::write(
        &full,
        r#"{"status":"received","slot_selected":false,"confirmed":false}"#,
    )
    .unwrap();
    ...
    assert!(
        !ws.has(&EducationStage::InterviewSlotSelected),
        "slot not selected"
    );
    assert!(
        !ws.has(&EducationStage::InterviewConfirmed),
        "not confirmed without slot"
    );
}
...
fn ard_prd_project_lifecycle_still_required() {
    ...
    assert!(
        !lc.has(&bcinr_pddl_lsp::lifecycle::LifecycleStage::PrdExists),
...
fn linkedin_candidate_not_admitted() {
    ...
    assert!(
        !ws.has(&EducationStage::LinkedInPublished),
        "no receipt → not published"
    );
    ...
    assert!(
        li_diag.is_some(),
        "should have LINKEDIN_RECEIPT_MISSING: {diags:?}"
    );
}
...
fn newsletter_need9_split() {
    ...
    let sections: String = (1..=9)
        .map(|i| format!("## Section {i}\n\nContent.\n\n"))
        .collect();
    write_file(
        &dir,
        "newsletter/issues/001-big.md",
        &format!("# Big Issue\n\nREVIEWED\n\n{sections}"),
    );
...
fn rust_lesson_requires_example_tests() {
    ...
    assert!(
        !ws.has(&EducationStage::RustExampleTestsPassed),
        "no test report"
    );
}
...
fn education_week_candidate_is_not_admitted() {
    ...
    let ws = scan(dir.path(), "sean");
    assert!(
        !ws.has(&EducationStage::EducationWeekPublished),
        "not admitted without receipt"
    );

    let gate = render_education_gate(&ws);
    assert!(
        !gate.contains("\"gate\":\"ADMITTED\""),
        "gate should not be ADMITTED: {gate}"
    );
}
...
fn explicit_education_week_admission_emits_receipt_and_ocel() {
    ...
    let projection = Pddl8Projection {
        domain_text: domain,
...
fn linkedin_external_publish_requires_platform_receipt() {
    ...
    assert!(
        receipt_diag.is_some(),
        "LINKEDIN_RECEIPT_MISSING expected: {diags:?}"
    );
}
...
fn youtube_publish_requires_artifact_or_platform_receipt() {
    ...
    assert!(
        !ws.has(&EducationStage::YouTubePublished),
        "no receipt → not published"
    );
}
...
fn education_week_publish_action_stays_within_8() {
    ...
    let action_start = domain
        .find("publish_education_week")
        .expect("action not found");
    let action_block = &domain[action_start..];
    let precond_start = action_block
        .find(":precondition")
        .expect("no :precondition");
    ...
    let pred_count = precond_text
        ...
        .filter(|l| {
            ...
            t.starts_with('(')
                && !t.starts_with("(:")
                && !t.starts_with("(and")
                && !t.starts_with("(:precondition")
        })
        .count();

    assert!(
        pred_count <= 8,
        "publish_education_week has {pred_count} preconditions, max is 8"
    );
    assert_eq!(
        pred_count, 8,
        "publish_education_week should have exactly 8 preconditions"
    );
}
...
fn education_mode_agent_next_step_is_mechanical() {
    ...
    let ws = scan(dir.path(), "sean");
    assert!(
        !ws.has(&EducationStage::NewsletterRestarted),
        "newsletter removed"
    );
...
fn end_to_end_sean_education_week_admitted() {
    ...
    assert!(
        ws2.has(&EducationStage::EducationWeekPublished),
        "education week admitted after receipt"
    );
    ...
    assert!(
        status.contains("\"admission_status\":\"ADMITTED\""),
        "status should show ADMITTED: {status}"
    );
}
