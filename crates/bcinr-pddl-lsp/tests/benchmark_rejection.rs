...
fn rejection_benchmarks() {
    ...
    samples.push(measure(
        "bounds_lifecycle_domain_real_check",
        100,
        ObservedOutcome::Accepted,
        || {
...
"#;
    ...
    // A-2. Empty lifecycle → OPEN (not admitted, not refused — no evidence yet)
    {
        ...
        let lc = lifecycle::scan(dir.path());
        samples.push(measure(
            "gate_open_empty_lifecycle",
            10_000,
            ObservedOutcome::Accepted,
            || {
                let gate = publish_gate::from_lifecycle(&lc);
                if gate.status_label() == "OPEN" {
                    ObservedOutcome::Accepted
                } else if gate.is_admitted() {
                    ObservedOutcome::Accepted
                } else {
                    ObservedOutcome::Rejected("PUBLISH_BLOCKED")
                }
            },
        ));
    }
    ...
    samples.push(measure(
        ...
        || {
            ...
            match state.request_slot("cargo build") {
                // → Denied
  ...
  (:goal (this_predicate_does_not_exist p-proj))
    ...
    // A-5. Fixture workspace: diagnostics sparse (education-week not yet admitted)
    {
        ...
        let full_count = {
            let empty_ws = education::scan(&TempDir::new().unwrap().into_path(), "sean");
        ...
        samples.push(measure(
            "education_diagnose_fixture_fewer_than_empty",
            10_000,
            ObservedOutcome::Accepted,
            || {
                let d = education::education_diagnostics(&ws);
                if d.len() < full_count {
                    ...
                    ObservedOutcome::Rejected("EDUCATION_FIXTURE_NOT_ADVANCING")
                }
            },
        ));
    }
    ...
    // R-10. 9 sections → split required
    {
        ...
        write_file(
            &dir,
            "newsletter/issues/big.md",
            &newsletter_with_n_sections(9),
        );
        samples.push(measure(
            "newsletter_need9_reject_9_sections",
            1_000,
    ...
    // A-6. 8 sections → accepted
    {
        ...
        write_file(
            &dir,
            "newsletter/issues/ok.md",
            &newsletter_with_n_sections(8),
        );
        samples.push(measure(
            "newsletter_need9_accept_8_sections",
            1_000,
            ObservedOutcome::Accepted,
            || match education::check_newsletter_need9(dir.path()) {
                None => ObservedOutcome::Accepted,
    ...
    // R-11. PRD exists without ADMITTED → PrdAdmitted missing
    {
        ...
        let lc = lifecycle::scan(dir.path());
        samples.push(measure(
            "lifecycle_reject_prd_not_admitted",
            10_000,
    ...
    // R-12. Receipt with goal_reached=false → Published not advanced
    {
        ...
        let lc = lifecycle::scan(dir.path());
        samples.push(measure(
            "lifecycle_reject_false_receipt",
            10_000,
            ObservedOutcome::Rejected("RECEIPT_INTEGRITY_ERROR"),
            || {
                use bcinr_pddl_lsp::lifecycle::LifecycleStage;
    ...
    samples.push(measure(
        "bounds_reject_empty_domain_text",
        10_000,
    ...
    for s in &samples {
        ...
        if matches!(s.outcome, ObservedOutcome::Andon(_)) {
            andon_count += 1;
        }
        println!(
            "{:<48} {:>6} {:>9.2}{} {:>9} {:>18} {:>5.1}ms  {}",
            s.name,
            s.iterations,
            s.value(),
            s.unit(),
    ...
    for s in &samples {
        if matches!(s.expected, ObservedOutcome::Andon(_)) {
            assert!(
                matches!(s.outcome, ObservedOutcome::Andon(_)),
                "ANDON — '{}' expected ANDON but got {:?}",
                s.name,
                s.outcome
            );
        }
    ...
    assert!(
        n100.ns_per_iter() <= n9.ns_per_iter() * 3,
        "Need9 not O(1): 100-task {}ns vs 9-task {}ns",
        n100.ns_per_iter(),
        n9.ns_per_iter()
    );
    ...
    assert!(
        bfs.ns_per_iter() < 5_000_000,
        "Pure BFS too slow: {}ms/iter (limit 5ms)",
        bfs.ns_per_iter() / 1_000_000
    );

    // Broker denial < 5µs
    assert!(
        get("broker_deny_double_acquire").ns_per_iter() < 5_000,
        "Broker denial too slow: {}ns",
        get("broker_deny_double_acquire").ns_per_iter()
    );

    // Total wall clock ≤ 8s
    assert!(
        wall_elapsed.as_millis() <= 8_000,
        "Rejection suite exceeded 8s: {}ms",
        wall_elapsed.as_millis()
    );

    println!("\nANDON count: {andon_count} expected ANDONs fired correctly.");
    println!(
        "All {} operations: {} pass, {} fail.",
        samples.len(),
        samples.len() - fail_count,
        fail_count
    );
    if let Some(bfs) = samples.iter().find(|s| s.name == "layer_bfs_find_plan_sat") {
        println!(
            "Pure BFS (cached GroundProblem): {:.2}{}",
            bfs.value(),
            bfs.unit()
        );
    }
    if let Some(pp) = samples.iter().find(|s| s.name == "plan_pipeline_lifecycle") {
        println!(
            "Full pipeline (parse+ground+BFS): {:.2}{}",
            pp.value(),
            pp.unit()
        );
    }
