use playground::legal_moves::{rook_attacks, bishop_attacks, queen_attacks, KNIGHT_MASKS, KING_MASKS};
...
fn generate_legal_targets(piece: &Piece, empty_mask: u64, enemy_mask: u64, own_mask: u64) -> u64 {
    ...
    if piece.kind == 0 {
        if piece.color == 0 {
            ...
            targets |= single;
            if (piece.square / 8) == 1 && single != 0 { targets |= (sq_mask << 16) & empty_mask; }
            targets |= (sq_mask << 7) & 0x7f7f7f7f7f7f7f7f & enemy_mask;
            targets |= (sq_mask << 9) & 0xfefefefefefefefe & enemy_mask;
        } else {
            let single = (sq_mask >> 8) & empty_mask;
            targets |= single;
            if (piece.square / 8) == 6 && single != 0 { targets |= (sq_mask >> 16) & empty_mask; }
            targets |= (sq_mask >> 9) & 0x7f7f7f7f7f7f7f7f & enemy_mask;
            targets |= (sq_mask >> 7) & 0xfefefefefefefefe & enemy_mask;
        }
    } else if piece.kind == 1 { targets = KNIGHT_MASKS[piece.square];
    } else if piece.kind == 5 { targets = KING_MASKS[piece.square];
    } else if piece.kind == 3 { targets = rook_attacks(sq_mask, empty_mask);
    } else if piece.kind == 2 { targets = bishop_attacks(sq_mask, empty_mask);
    } else if piece.kind == 4 { targets = queen_attacks(sq_mask, empty_mask); }
...
fn get_bitboards(pieces: &[Piece; 32]) -> [u64; 12] {
    let mut bb = [0u64; 12];
    for p in pieces {
        if p.alive { bb[(p.color * 6 + p.kind) as usize] |= 1u64 << p.square; }
    }
    bb
}
...
fn branchless_static_evaluation(pieces: &[Piece; 32]) -> i32 {
    let mut white_score = 0; let mut black_score = 0;
    let vals = [100, 320, 330, 500, 900, 20000];
    for p in pieces {
        if p.alive {
            let val = vals[p.kind as usize];
            if p.color == 0 { white_score += val; } else { black_score += val; }
        }
    }
...
fn main() {
    let initial_setup = [
        (3,0,0), (1,0,1), (2,0,2), (4,0,3), (5,0,4), (2,0,5), (1,0,6), (3,0,7),
        (0,0,8), (0,0,9), (0,0,10), (0,0,11), (0,0,12), (0,0,13), (0,0,14), (0,0,15),
        (3,1,56), (1,1,57), (2,1,58), (4,1,59), (5,1,60), (2,1,61), (1,1,62), (3,1,63),
        (0,1,48), (0,1,49), (0,1,50), (0,1,51), (0,1,52), (0,1,53), (0,1,54), (0,1,55),
    ];

    let mut start_pieces = [Piece { kind: 0, color: 0, alive: false, square: 0 }; 32];
    for i in 0..32 {
        start_pieces[i] = Piece { kind: initial_setup[i].0, color: initial_setup[i].1, alive: true, square: initial_setup[i].2 };
    }
    ...
    (0..1_000_000).into_par_iter().for_each(|_| {
        if start_time.elapsed().as_secs_f64() >= 1.0 { return; }
        ...
        while !game_over && turn_counter <= 100 {
            ...
            let mut white_mask = 0u64; let mut black_mask = 0u64;
            for i in 0..6 { white_mask |= bb[i]; black_mask |= bb[i+6]; }
            ...
            for idx in 0..32 {
                ...
                if p.alive && p.color == color_filter {
                    ...
                    while t != 0 {
                        ...
                        for v_idx in 0..32 {
                            if test_pieces[v_idx].alive && test_pieces[v_idx].square == sq && test_pieces[v_idx].color != color_filter {
                                test_pieces[v_idx].alive = false; break;
                            }
                        }
                        ...
                        if !move_found {
                            best_score = shannon_target; best_move = (idx, sq); move_found = true;
                        } else if is_white && shannon_target > best_score {
                            best_score = shannon_target; best_move = (idx, sq);
                        } else if !is_white && shannon_target < best_score {
                            best_score = shannon_target; best_move = (idx, sq);
                        }
                    }
            ...
            for v_idx in 0..32 {
                if pieces[v_idx].alive && pieces[v_idx].square == target_square {
                    ...
                    if pieces[v_idx].kind == 5 { game_over = true; }
                    break;
                }
            }
