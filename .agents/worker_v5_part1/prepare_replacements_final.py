...
# Decoupled / Mathematically Correct references and branchless implementations database:
logic_db = {
    ...
    "bset_u64": (
        """    let start = aux & 0x3F;
    val | (1u64.wrapping_shl(start as u32))""",
        """        let bit = aux & 63;
        let mut res = val;
        if ((val >> bit) & 1) == 0 {
            res ^= 1u64 << bit;
...
for algo in partition1:
    ...
    # Replace # CONTRACT with # Branchless Contract if needed
    doc_target = ""
    doc_replacement = ""
    if "# CONTRACT" in content:
        doc_target = "# CONTRACT"
        doc_replacement = "# Branchless Contract"
...
with open("/Users/sac/bcinr/.agents/worker_v5_part1/replacements_final.json", "w") as f:
    json.dump(report, f, indent=2)

print("Done. Generated replacements_final.json with", len(report), "entries.")
