...
fn bench_bloom_scan_pipeline(c: &mut Criterion) {
    ...
    let buffer = [b'a'; 64];
    c.bench_function("BloomScanPipeline", |b| {
        b.iter(|| pipeline.process_64(black_box(&buffer), black_box(b'a')))
    });
}
...
fn bench_priority_petri_engine(c: &mut Criterion) {
    ...
    c.bench_function("PriorityPetriEngine", |b| b.iter(|| engine.step()));
}

fn bench_lockfree_mpmc(c: &mut Criterion) {
    let ring = LockFreeMpmcRing::<u32, 16>::new_checked().unwrap();
    c.bench_function("LockFreeMpmcRing_Push", |b| {
        b.iter(|| ring.push_t1(black_box(42)))
    });
    c.bench_function("LockFreeMpmcRing_Pop", |b| b.iter(|| ring.pop_t1()));
}

fn bench_autonomic_exhaustion_arena(c: &mut Criterion) {
    let mut arena = AutonomicExhaustionArena::new(1024, 100);
    c.bench_function("AutonomicExhaustionArena", |b| {
        b.iter(|| arena.alloc_aligned_t1(black_box(50)))
    });
}

fn bench_bit_transcoder(c: &mut Criterion) {
    let transcoder = BitTranscoder::new(0x0F0F0F0F0F0F0F0F, 0xF0F0F0F0F0F0F0F0);
    c.bench_function("BitTranscoder", |b| {
        b.iter(|| transcoder.transcode(black_box(0x1234567890ABCDEF)))
    });
}
...
fn bench_constant_shape_policy_dfa(c: &mut Criterion) {
    ...
    c.bench_function("ConstantShapePolicyDfa", |b| {
        b.iter(|| dfa.step(black_box(0), black_box(b'a')))
    });
}

fn bench_deterministic_substrate_receipt(c: &mut Criterion) {
    let mut receipt = DeterministicSubstrateReceipt::new();
    c.bench_function("DeterministicSubstrateReceipt", |b| {
        b.iter(|| {
...
fn bench_bounded_spsc_multicast(c: &mut Criterion) {
    ...
    c.bench_function("BoundedSpscMulticast", |b| {
        b.iter(|| multicast.broadcast_partial())
    });
}
...
fn bench_wcet_fiber(c: &mut Criterion) {
    ...
    let events = [1, 2, 3];
    c.bench_function("WcetFiber", |b| {
        b.iter(|| fiber.execute_budget_fixed(black_box(&events)))
    });
}

fn bench_register_sql(c: &mut Criterion) {
    let mut data = [8, 7, 6, 5, 4, 3, 2, 1];
    c.bench_function("RegisterEngine", |b| {
        b.iter(|| RegisterEngine::sort_and_filter(black_box(&mut data), black_box(5)))
    });
}

fn bench_matrix_lru(c: &mut Criterion) {
    let mut lru = MatrixLru::<4>::new();
    c.bench_function("MatrixLru", |b| {
        b.iter(|| {
...
fn bench_chacha_sponge(c: &mut Criterion) {
    ...
    c.bench_function("ChaChaSponge", |b| {
        b.iter(|| {
...
fn bench_swar_quotient(c: &mut Criterion) {
    ...
    c.bench_function("SwarQuotientFilter", |b| {
        b.iter(|| {
...
fn bench_bitonic_pq(c: &mut Criterion) {
    ...
    c.bench_function("BitonicPriorityQueue8", |b| {
        b.iter(|| {
...
fn bench_hazard_shield(c: &mut Criterion) {
    ...
    c.bench_function("HazardShield", |b| {
        b.iter(|| {
...
fn bench_radix_trie(c: &mut Criterion) {
    ...
    node.children[0] = 42;
    c.bench_function("RadixTrieNode", |b| b.iter(|| node.lookup(black_box(b'a'))));
}

fn bench_consensus_bft(c: &mut Criterion) {
    let mut bft = FixedConsensus::<2>::new();
    c.bench_function("FixedConsensus", |b| {
        b.iter(|| {
