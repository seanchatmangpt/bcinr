...
///
/// ```
/// # use bcinr_core::logic::parse::skip_whitespace;
...
///
/// ```
/// # use bcinr_core::logic::parse::parse_hex_u32;
...
/// ```
#[inline(always)]
pub(crate) fn parse_hex_u32(bytes: &[u8]) -> Result<u32, &'static str> {
    if bytes.is_empty() {
...
///
/// ```
/// # use bcinr_core::logic::parse::parse_decimal_u64;
...
/// ```
#[inline(always)]
pub(crate) fn parse_decimal_u64(bytes: &[u8]) -> Result<u64, &'static str> {
    if bytes.is_empty() {
        return Err("empty input");
    }
...
mod benches {
    ...
    #[bench]
    fn bench_skip_whitespace_short(b: &mut Bencher) {
        let input = b"   hello";
        b.iter(|| skip_whitespace(input));
    }

    #[bench]
    fn bench_skip_whitespace_long(b: &mut Bencher) {
        let input = b"                    hello";
        b.iter(|| skip_whitespace(input));
    }

    #[bench]
    fn bench_parse_hex_u32_typical(b: &mut Bencher) {
        let input = b"deadbeef";
        b.iter(|| parse_hex_u32(input));
    }

    #[bench]
    fn bench_parse_hex_u32_single(b: &mut Bencher) {
        let input = b"f";
        b.iter(|| parse_hex_u32(input));
    }

    #[bench]
    fn bench_parse_hex_u32_max(b: &mut Bencher) {
        let input = b"ffffffff";
        b.iter(|| parse_hex_u32(input));
    }

    #[bench]
    fn bench_parse_decimal_u64_small(b: &mut Bencher) {
        let input = b"123";
        b.iter(|| parse_decimal_u64(input));
    }

    #[bench]
    fn bench_parse_decimal_u64_medium(b: &mut Bencher) {
        let input = b"1000000000";
        b.iter(|| parse_decimal_u64(input));
    }

    #[bench]
    fn bench_parse_decimal_u64_max(b: &mut Bencher) {
