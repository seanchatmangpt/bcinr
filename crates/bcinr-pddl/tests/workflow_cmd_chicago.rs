...
impl PolicyIdentity for CommandLimitPolicy {
    fn root(&self) -> PolicySetRoot {
        PolicySetRoot::hash_parts(&[
            b"command-limit-policy-v1",
            &self.max_commands.to_le_bytes(),
        ])
    }
}
...
chicago_tdd_tools::test!(application_compiles_control_flow_and_advances_only_after_observed_effects, {
    ...
    let verified = workflow.plan(&state).expect("planning should earn standing");
    let typed = binding
        .bind_plan(&verified)
    ...
    assert_eq!(typed.standing(), CognitiveExecutionStanding::WitnessedConcurrentStrips);
    assert_eq!(proposal.commands().len(), 2);
    assert_eq!(broker.admitted_roots(), &[proposal.root()]);
    assert_eq!(cursor.next_tick(), None);
    assert!(cursor.commands().iter().all(|command| matches!(
        command.progress,
...
chicago_tdd_tools::test!(transport_replay_restores_only_matching_trusted_evidence, {
    ...
    let json = serde_json::to_string(&trusted.erase_for_transport())
...
chicago_tdd_tools::test!(receipt_chain_connects_decision_dispatch_effect_and_cursor, {
    // Arrange: compile and broker a real application plan.
    let (mut workflow, state, observation, goal, binding) = arrange_compiled_workflow();
    let verified = workflow.plan(&state).expect("planning should succeed");
    let typed = binding.bind_plan(&verified).expect("binding should succeed");
    let envelope = workflow
        .manufacture_plan_envelope(
            &verified,
            observation.root(),
    ...
    let mut cursor = WorkflowCursor::from_proposal(&proposal);
    let ready = cursor.next_ready();
    cursor.record_admission(&admission).expect("cursor admission");
...
chicago_tdd_tools::test!(residual_reconciliation_preserves_only_unchanged_observations, {
