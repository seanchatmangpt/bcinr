//! Branchless sliding-piece attacks via Kogge-Stone parallel prefix
//! (ported from `playground/src/legal_moves.rs`). CC = 1, no allocations.

/// Kogge-Stone south ray fill.
#[inline(always)]
#[must_use]
...
/// Kogge-Stone east ray fill.
#[inline(always)]
#[must_use]
pub fn east_attacks(mut gen: u64, mut pro: u64) -> u64 {
    pro &= 0xfefe_fefe_fefe_fefe;
    gen |= pro & (gen << 1);
    pro &= pro << 1;
    gen |= pro & (gen << 2);
    pro &= pro << 2;
    gen |= pro & (gen << 4);
    (gen << 1) & 0xfefe_fefe_fefe_fefe
}

/// Kogge-Stone west ray fill.
#[inline(always)]
#[must_use]
pub fn west_attacks(mut gen: u64, mut pro: u64) -> u64 {
    pro &= 0x7f7f_7f7f_7f7f_7f7f;
    gen |= pro & (gen >> 1);
    pro &= pro >> 1;
    gen |= pro & (gen >> 2);
    pro &= pro >> 2;
    gen |= pro & (gen >> 4);
    (gen >> 1) & 0x7f7f_7f7f_7f7f_7f7f
}

/// Kogge-Stone north-east ray fill.
#[inline(always)]
#[must_use]
pub fn no_ea_attacks(mut gen: u64, mut pro: u64) -> u64 {
    ...
    pro &= pro << 18;
    gen |= pro & (gen << 36);
    (gen << 9) & 0xfefe_fefe_fefe_fefe
}

/// Kogge-Stone south-east ray fill.
#[inline(always)]
#[must_use]
pub fn so_ea_attacks(mut gen: u64, mut pro: u64) -> u64 {
    ...
    pro &= pro >> 14;
    gen |= pro & (gen >> 28);
    (gen >> 7) & 0xfefe_fefe_fefe_fefe
}

/// Kogge-Stone north-west ray fill.
#[inline(always)]
#[must_use]
pub fn no_we_attacks(mut gen: u64, mut pro: u64) -> u64 {
    ...
    pro &= pro << 14;
    gen |= pro & (gen << 28);
    (gen << 7) & 0x7f7f_7f7f_7f7f_7f7f
}

/// Kogge-Stone south-west ray fill.
#[inline(always)]
#[must_use]
pub fn so_we_attacks(mut gen: u64, mut pro: u64) -> u64 {
    ...
    pro &= pro >> 18;
    gen |= pro & (gen >> 36);
    (gen >> 9) & 0x7f7f_7f7f_7f7f_7f7f
}

/// Rook attacks from `sq` given `empty` squares.
#[inline(always)]
#[must_use]
...
/// Bishop attacks from `sq` given `empty` squares.
#[inline(always)]
#[must_use]
...
/// Queen attacks from `sq` given `empty` squares.
#[inline(always)]
#[must_use]
...
const fn compute_knight_attacks() -> [u64; 64] {
    ...
    while i < 64 {
        let sq = 1u64 << i;
        let mut attacks = 0;
        let not_a = 0xfefe_fefe_fefe_fefe;
        let not_h = 0x7f7f_7f7f_7f7f_7f7f;
        let not_ab = 0xfcfc_fcfc_fcfc_fcfc;
        let not_gh = 0x3f3f_3f3f_3f3f_3f3f;
        attacks |= (sq << 17) & not_a;
...
/// Precomputed knight attack masks indexed by square.
pub static KNIGHT_MASKS: [u64; 64] = compute_knight_attacks();
/// Precomputed king attack masks indexed by square.
