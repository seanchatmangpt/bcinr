...
fn test_event_ordering_preserved_ocel_chain() {
    ...
    log_correct.record_op_fired(run_id, 0, 0, 1).unwrap(); // A proposes X
    log_correct.record_op_fired(run_id, 1, 1, 1).unwrap(); // B votes X
    log_correct.record_op_fired(run_id, 2, 2, 1).unwrap(); // C votes X
    ...
    log_reordered.record_op_fired(run_id, 1, 0, 1).unwrap(); // B votes X (moved first)
    log_reordered.record_op_fired(run_id, 0, 1, 1).unwrap(); // A proposes X (moved second)
