...
/// Bound on how many times a choice-graph node may be revisited when
/// unrolling cyclic paths for language enumeration.
...
/// and by a per-node revisit cap for cycles.
fn choice_graph_language(
    ...
    cg_dfs(&adj, child_langs, start, end, n_children, max_len, &mut prefix, &mut visits, &mut result);
    result
}
...
fn cg_dfs(
    ...
    if visits[node] > MAX_CYCLE_UNROLL || prefix.len() > max_len {
        return;
    }
    visits[node] += 1;
    if node < n_children {
        for seq in &child_langs[node] {
            ...
            for &nxt in &adj[node] {
                cg_dfs(adj, child_langs, nxt, end, n_children, max_len, prefix, visits, result);
            }
