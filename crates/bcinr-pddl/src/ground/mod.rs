...
impl GroundTemporalProblem {
    ...
    pub fn find_temporal_plan_with_fn_overrides(
        ...
        // Real trajectory-constraint enforcement for `domain.constraints`
        // (see the `trajectory_constraints` field's doc comment) — a fresh
        // policy per call, since its monitor states are mutable and this
        // function may be invoked more than once (e.g.
        // `schedule_analysis::replan_with_perturbed_capacity` calling this
        // repeatedly with different `overrides`). `policy_prev_state` is
        // updated once per outer-loop tick, at the same granularity the
        // ad hoc `self.constraints` check just above already uses.
        let mut policy = trajectory_policy::TrajectoryPolicy::new(&[], &self.trajectory_constraints);
        ...
        for _iteration in 0..PDDL8_MAX_PLAN_DEPTH {
            ...
            // Domain-level hard `:constraints`, checked for real via
            // `trajectory_policy::TrajectoryPolicy` (see the
            // `trajectory_constraints` field's doc comment). A hard-constraint
            // violation here is unrecoverable for this single greedy
            // trajectory — same fatal treatment as the ad hoc check above —
            // so this trajectory is dead exactly like every other
            // `trajectory_dead = true` site in this loop.
            if policy
                .step(
                    &policy_prev_state,
                    None,
