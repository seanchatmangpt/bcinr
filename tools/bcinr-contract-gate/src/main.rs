    ...
    fn visit_item_fn(&mut self, i: &'ast ItemFn) {
        ...
        if matches!(i.vis, Visibility::Public(_)) {
            ...
            if !cv.forbidden_ops.is_empty()
                && (str_has_substr(&name, "add_bitwise") || str_has_substr(&name, "sub_bitwise"))
            {
                self.errors.push(format!(
...
fn main() {
    ...
    let mut missing_u64: Vec<&PublicFunction> = visitor
        .public_functions
        .iter()
        .filter(|f| {
            let p = f.path.to_string_lossy();
            !str_ends_with(&p, "/mod.rs")
    ...
    let with_u64 = visitor
        .public_functions
        .iter()
        .filter(|f| f.has_u64_contract)
        .count();
