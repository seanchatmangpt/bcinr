...
fn invariant_prd_admission_true_case() {
    ...
    let record = invariants::check_all(&lc, &bounds::check_lifecycle_domain())
        .into_iter()
        .find(|r| r.id == "prd_admission")
        .unwrap();
    // TRUE: PRD admitted → true_case = PASS
    assert_eq!(
        record.true_case.outcome,
        ProbeOutcome::Pass,
        "TRUE case failed: {:?}",
        record.true_case.observed
    );
}
...
fn invariant_prd_admission_false_case() {
    ...
    assert!(
        record.false_case.outcome.is_blocking(),
        "FALSE case must block: {:?}",
        record.false_case.outcome
    );
}
...
fn invariant_prd_admission_counterfactual_defined() {
    ...
    let record = invariants::check_all(&lc, &bounds::check_lifecycle_domain())
        ...
        .unwrap();
    assert!(
        !record.counterfactuals.is_empty(),
...
fn invariant_need9_true_false_cf() {
    ...
    assert!(
        record.false_case.outcome.is_blocking(),
        "FALSE(9 tasks) must block: {:?}",
        record.false_case.outcome
    );

    // COUNTERFACTUAL: boundary mutation defined
    let boundary_cf = record
        .counterfactuals
        .iter()
        .find(|cf| cf.mutation == CounterfactualMutation::BoundaryMutation);
    assert!(
        boundary_cf.is_some(),
        "BoundaryMutation counterfactual required"
    );
    assert!(
        boundary_cf.unwrap().result.outcome.is_blocking(),
        "CF must block: got {:?}",
        boundary_cf.unwrap().result.outcome
    );
...
fn invariant_need9_verdict_is_pass() {
    ...
    assert_eq!(
        ...
        "Need9 invariant must be PASS, got {:?}",
        record.verdict
    );
    assert!(
        record.is_admitted(),
        "Need9 invariant must be fully admitted"
    );
}
...
fn invariant_bound_checks_stub_detection() {
    ...
    let record = invariants::check_all(&lc, &empty_report)
        .into_iter()
        .find(|r| r.id == "bound_checks_ran")
        .unwrap();

    assert!(
        record.true_case.outcome.is_blocking(),
        "Empty report true_case must be STOP/ANDON, got {:?}",
        record.true_case.outcome
    );
    assert_ne!(
        record.verdict,
    ...
    let disable_cf = record
        .counterfactuals
        .iter()
        .find(|cf| cf.mutation == CounterfactualMutation::DisableChecker);
    assert!(
        disable_cf.is_some(),
        "DisableChecker counterfactual required"
    );
}
...
fn invariant_bound_checks_real_run_is_pass() {
    ...
    assert!(
        w.summary.as_deref().unwrap_or("").contains("checks_run="),
        "WITNESS summary must include checks_run count: {:?}",
        w.summary
    );
}
...
fn invariant_publish_false_case_no_receipt() {
    ...
    let record = invariants::check_all(&lc, &bounds::check_lifecycle_domain())
        .into_iter()
        .find(|r| r.id == "publish_requires_receipt")
        .unwrap();

    // FALSE: no receipt → correctly blocked
    assert!(
        record.false_case.outcome.is_blocking(),
        "FALSE(no receipt) must block: {:?}",
        record.false_case.outcome
    );
}
...
fn invariant_publish_counterfactuals_corrupt_and_remove() {
    ...
    let has_corrupt = record
        .counterfactuals
        .iter()
        .any(|cf| cf.mutation == CounterfactualMutation::CorruptEvidence);
    let has_remove = record
        .counterfactuals
        .iter()
        .any(|cf| cf.mutation == CounterfactualMutation::RemoveEvidence);
    assert!(
        has_corrupt,
        "CorruptEvidence counterfactual required (flip goal_reached)"
    );
    assert!(
        has_remove,
        "RemoveEvidence counterfactual required (delete receipt)"
    );

    // Both must be blocking
    for cf in &record.counterfactuals {
        assert!(
            cf.result.outcome.is_blocking(),
            "CF '{}' must block, got {:?}",
            cf.description,
            cf.result.outcome
        );
...
fn invariant_candidate_not_admitted_has_authority_swap_cf() {
    ...
    let record = invariants::check_all(&lc, &bounds::check_lifecycle_domain())
        .into_iter()
        .find(|r| r.id == "candidate_not_admitted")
        .unwrap();
    ...
    assert!(
        auth_cf.unwrap().result.outcome.is_blocking(),
        "AuthoritySwap must block: got {:?}",
        auth_cf.unwrap().result.outcome
    );
...
fn all_invariants_have_witness_and_repair() {
    ...
    assert!(
        !records.is_empty(),
        "check_all must return at least one invariant"
    );
    for r in &records {
        assert!(
            r.witness.is_some(),
            "Invariant '{}' missing WITNESS — not admitted",
            r.id
        );
        assert!(
            r.repair.is_some(),
            "Invariant '{}' missing REPAIR — not admitted",
            r.id
        );
        assert!(
            !r.counterfactuals.is_empty(),
            "Invariant '{}' has no COUNTERFACTUAL probes — not admitted",
            r.id
        );
    }
}
...
fn render_invariant_table_is_valid_json() {
    ...
    let parsed: serde_json::Value =
        serde_json::from_str(&json).expect("render_invariant_table must produce valid JSON");
    ...
    for entry in arr {
        ...
        assert!(
            entry.get("counterfactuals").is_some(),
            "must have counterfactuals"
        );
        assert!(
            entry.get("witness_present").is_some(),
            "must have witness_present"
        );
        assert!(
            entry.get("repair_present").is_some(),
            "must have repair_present"
        );
