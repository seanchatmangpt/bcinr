...
pub fn select_bit_u64(x: u64, nth: usize) -> usize {
    ...
    while left < right {
        ...
        if count_at_mid <= nth { left = mid + 1; } else { right = mid; }
    }
    left - 1
}
...
pub fn jaccard_u64_slices(a: &[u64], b: &[u64]) -> f32 {
    ...
    for i in 0..a.len() {
        intersection_count += (a[i] & b[i]).count_ones();
        union_count += (a[i] | b[i]).count_ones();
    }
    if union_count == 0 { 1.0 } else { intersection_count as f32 / union_count as f32 }
}
