...
use crate::wf_net::{Label, WfNet};

/// Machine-readable reason a WF-net (or sub-net) was refused as non-separable.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum RefusalReason {
    /// Not free-choice (Def 3.4). Every separable WF-net is free-choice.
    NonFreeChoice { transitions: (String, String) },
    /// No base case and no valid partition at this recursion level.
...
impl std::fmt::Display for RefusalReason {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::NonFreeChoice { transitions: (a, b) } => write!(
                f,
...
pub fn convert_and_verify(net: &WfNet, budget: usize, max_len: usize) -> Result<Powl2Model, Refusal> {
    ...
    if denotational != replayed {
        return Err(Refusal {
            reason: RefusalReason::LanguageMismatch { checked_len: max_len },
...
fn convert_rec(net: &WfNet, depth: usize, budget: usize) -> Result<Powl2Model, Refusal> {
    ...
    if sm.len() > 1 && is_concurrency_hiding(net, &sm) && sm_makes_progress(net, &sm) {
        ...
        return Ok(Powl2Model::ChoiceGraph { children, edges, start, end });
    }
...
/// Convert one child part. A singleton part is always a single transition,
/// whose POWL representation is `Activity`/`Silent` directly — this also
/// terminates the recursion on self-loops, whose projection would otherwise
/// reproduce a bare-loop sub-net indefinitely.
fn convert_child(
    net: &WfNet,
...
fn partition_sm(net: &WfNet) -> Vec<BTreeSet<String>> {
    ...
    for tsplit in net.transitions().keys() {
        ...
        let reach: BTreeMap<String, BTreeSet<String>> =
            outs.iter().map(|p| (p.clone(), net.fwd_restricted(p, tsplit))).collect();
    ...
    for tjoin in net.transitions().keys() {
        ...
        let reach: BTreeMap<String, BTreeSet<String>> =
            ins.iter().map(|p| (p.clone(), net.bwd_restricted(p, tjoin))).collect();
        let mut threads = BTreeSet::new();
        for t in net.transitions().keys() {
            if t == tjoin {
                continue;
...
fn is_concurrency_hiding(net: &WfNet, parts: &[BTreeSet<String>]) -> bool {
    parts.iter().all(|part| net.entry_places(part).len() == 1 && net.exit_places(part).len() == 1)
}
...
fn project_mg(net: &WfNet, part: &BTreeSet<String>) -> WfNet {
    ...
    let mut places: BTreeSet<String> =
        touching.iter().filter(|p| !entry.contains(*p) && !exit.contains(*p)).cloned().collect();
    ...
    let transitions: BTreeMap<String, Label> = part.iter().map(|t| (t.clone(), net.label(t))).collect();
...
fn normalize(
    ...
    WfNet::new(places, transitions, pt, tp, source, sink).expect("projection + normalization yields a valid WF-net")
}
