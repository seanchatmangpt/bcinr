...
use bcinr_powl::wf_to_powl::{convert, Refusal};
...
/// Materialize a temporal plan as an actual `WfNet` via recomposition.
#[must_use]
pub fn temporal_plan_to_wf_net(problem: &GroundTemporalProblem, plan: &TemporalPlan) -> WfNet {
...
pub fn temporal_plan_to_powl2(
    ...
    let net = temporal_plan_to_wf_net(problem, plan);
    convert(&net)
}
