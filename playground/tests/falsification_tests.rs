...
use playground::gnn::{hoeg_gnn_forward, BinarizedGnnLayer};
...
// --- HOEG Mutants ---
fn mutant1_compile_hoeg_matrix(node_types: &[u16], features: &[u64], connections: &[u64], out: &mut [Hoeg64Node]) -> Result<usize, &'static str> {
...
fn mutant2_compile_hoeg_matrix(node_types: &[u16], features: &[u64], connections: &[u64], out: &mut [Hoeg64Node]) -> Result<usize, &'static str> {
    let res = compile_hoeg_matrix(node_types, features, connections, out);
    for i in 0..node_types.len() {
...
fn mutant3_compile_hoeg_matrix(node_types: &[u16], features: &[u64], connections: &[u64], out: &mut [Hoeg64Node]) -> Result<usize, &'static str> {
    let res = compile_hoeg_matrix(node_types, features, connections, out);
    for i in 0..node_types.len() {
...
// --- GNN Mutants ---
fn mutant1_hoeg_gnn_forward(nodes: &[Hoeg64Node], layer: &BinarizedGnnLayer, out_features: &mut [u64]) -> Result<(), &'static str> {
    if nodes.len() != out_features.len() { return Err("Output buffer size mismatch."); }
...
fn mutant2_hoeg_gnn_forward(nodes: &[Hoeg64Node], layer: &BinarizedGnnLayer, out_features: &mut [u64]) -> Result<(), &'static str> {
    if nodes.len() != out_features.len() { return Err("Output buffer size mismatch."); }
    for i in 0..nodes.len() {
        let node = &nodes[i];
...
fn mutant3_hoeg_gnn_forward(nodes: &[Hoeg64Node], layer: &BinarizedGnnLayer, out_features: &mut [u64]) -> Result<(), &'static str> {
    if nodes.len() != out_features.len() { return Err("Output buffer size mismatch."); }
    for i in 0..nodes.len() {
        let node = &nodes[i];
        let mut aggregated_features = node.feature_mask;
...
// --- TEKG Mutants ---
fn mutant1_compile_snapshot_chain(entity_id: u16, update_timestamps: &[u64], out: &mut [Tekg64Node]) -> Result<usize, &'static str> {
...
fn mutant2_compile_snapshot_chain(entity_id: u16, update_timestamps: &[u64], out: &mut [Tekg64Node]) -> Result<usize, &'static str> {
    let res = compile_snapshot_chain(entity_id, update_timestamps, out);
    if let Ok(count) = res {
        for i in 1..count {
...
fn mutant3_compile_snapshot_chain(entity_id: u16, update_timestamps: &[u64], out: &mut [Tekg64Node]) -> Result<usize, &'static str> {
    let res = compile_snapshot_chain(entity_id, update_timestamps, out);
    if let Ok(count) = res {
        for i in 1..count {
...
fn test_falsify_hoeg_mutants() {
    ...
    let mut out_real = [Hoeg64Node { feature_mask: 0, adjacency_mask: 0, node_id: 0, node_type_hash: 0, _pad: [0; 44] }; 3];
...
fn test_falsify_gnn_mutants() {
    ...
    let nodes = [
        Hoeg64Node { feature_mask: 0b1, adjacency_mask: 0b1, node_id: 0, node_type_hash: 1, _pad: [0; 44] },
        // Node 1 will use weight 1 in real, weight 0 in mutant 2
        Hoeg64Node { feature_mask: 0, adjacency_mask: 0, node_id: 1, node_type_hash: 1, _pad: [0; 44] },
    ];
...
fn test_falsify_tekg_mutants() {
    ...
    let mut out_real = [Tekg64Node { timestamp_ns: 0, rel_mask: 0, node_id: 0, parent_id: 0, prev_snapshot_id: 0, label: TekgLabel::Log, _pad: [0; 41] }; 4];
    ...
    if !tekg_nodes_eq(&out_real, &out_m1) { m1_caught = true; }

    let mut out_m2 = out_real.clone();
    mutant2_compile_snapshot_chain(entity_id, &timestamps, &mut out_m2).unwrap();
    if !tekg_nodes_eq(&out_real, &out_m2) { m2_caught = true; }

    let mut out_m3 = out_real.clone();
    mutant3_compile_snapshot_chain(entity_id, &timestamps, &mut out_m3).unwrap();
    if !tekg_nodes_eq(&out_real, &out_m3) { m3_caught = true; }
