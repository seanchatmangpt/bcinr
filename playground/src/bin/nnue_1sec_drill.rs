...
#[derive(Clone, Copy)]
struct Piece {
    kind: u8, // 0:Pawn, 1:Knight, 2:Bishop, 3:Rook, 4:Queen, 5:King
...
fn get_bitboards(pieces: &[Piece; 32]) -> [u64; 12] {
    ...
    for p in pieces {
        if p.alive {
            let idx = (p.color * 6 + p.kind) as usize;
            bb[idx] |= 1u64 << p.square;
        }
    }
    bb
}
...
fn main() {
    ...
    while start_time.elapsed().as_secs_f64() < 1.0 {
        ...
        while !game_over && turn_counter <= 100 {
            ...
            for idx in 0..32 {
                ...
                if p.alive && p.color == color_filter {
                    ...
                    let mut t = targets;
                    while t != 0 {
                        // Extract lowest set bit (using fast tzcnt intrinsic on M3)
            ...
            for v_idx in 0..32 {
                if pieces[v_idx].alive && pieces[v_idx].square == target_square {
                    ...
                    if pieces[v_idx].kind == 5 { game_over = true; } // King captured
                    break;
                }
            }
